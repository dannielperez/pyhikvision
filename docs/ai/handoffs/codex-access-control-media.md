...
- validations: `pytest -q` OK 91 passed; changed-file `ruff check` and `ruff format --check` OK
...
- safety: no device call, secret, deployment, release, or merge; RTSP errors never render the credential-bearing URL; native capture rejects non-JPEG success bodies and the subprocess obeys the caller's total timeout
