...
- outcome: native JPEG responses stream in 64 KiB chunks, stop above 8 MiB,
  enforce the caller's monotonic content deadline, and close responses on every exit
...
- validation: `97 passed`; scoped fatal/static checks and `git diff --check` pass
